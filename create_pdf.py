...
def risk_return(SPY, table_name):
    t_list = table_name.iloc[:, :-1].sum(axis = 1)
...
def create_pdf():
    ...
    for name in unique_sector['sector']:
        ...
        plt.pie(indus_table['Percentage'], autopct='%.2f%%')
        pie_title = "Pie Chart of " + name + " Percentage"
        plt.title(pie_title, fontweight='bold')
        plt.legend(indus_table['Industry'], loc='best')
        image_name = 'industry' + str(name_num)+'.png'
        plt.savefig(image_name, dpi=300, bbox_inches="tight")

        if name_num == 1:
            ypo = 155
            pdf.image(image_name, x=140, y=ypo, w=40, type='', link='')
            plt.clf()
            ...
            if len(indus_per(unique_sector['sector'][name_num - 2])) < 2:
                ...
                ypo = ypo + times
                print("Width:", width)
                print("With line 34, currently,", image_name, "is located at x = 140 and y =", ypo)
                if ypo > 250:
                    page_in = 1
                    ypo = 17
                if (width == 41) & (ypo == 17):
                    ypo = 25
                    width = 45
                pdf.image(image_name, x=140, y=ypo, w=width, type='', link='')
                plt.clf()
                ...
                ypo = ypo + times
                print("Width:", width)
                print("With line 40, currently,", image_name, "is located at x = 140 and y =", ypo)
                if ypo > 250:
                    page_in = 1
                    ypo = 17
                if (width == 41) & (ypo == 17):
    ...
    plt.plot(final_portfolio['stock_date'], table_name['Portfolio'], label = 'Portfolio', linewidth=1, alpha=0.5)
    plt.legend()
    ...
    pdf.image("return.png", x=36, y=135, w=140, type='', link='')
    plt.clf()
